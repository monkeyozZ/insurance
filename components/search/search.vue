<template>
	<view class="content">
		<view class="search-box">
			...
			<mSearch class="mSearch-input-box" @back="backFn" :mode="2" button="inside" :placeholder="defaultKeyword" @search="doSearch(false)" @confirm="doSearch(false)" v-model="keyword"></mSearch>
...
<script>
	...
	import {getFilterCompany} from '@/api/product.js';
	export default {
		data() {
			return {
				defaultKeyword: "",
		...
		methods: {
			...
			loadHotKeyword() {
				getFilterCompany().then((res) => {
					res.data.data.map((item) => {
						this.hotKeywordList.push(item.name)
					})
					
				})
				//定义热门搜索关键字，可以自己实现ajax请求数据再赋值
				// this.hotKeywordList = ['中国平安', '中国人寿', '新华保险', '我也不知道', '什么保险', '都是骗人的', '你个蠢货', '上当了吧', '哈哈哈笑死我了'];
			}, 
			//监听输入
			inputChange(event) {
				//兼容引入组件时传入参数情况
			...
			doSearch(key) {
				...
				this.$emit('refresh', this.keyword)
				/* uni.showToast({
					title: key,
					icon: 'none',
					duration: 2000
...
<style>
	view{display:block;}
	.search-box {margin:0 auto;background-color:rgb(242,242,242);padding:15upx 2.5%;display:flex;justify-content:space-between;}
